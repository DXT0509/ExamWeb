create or replace function public.reorder_exam_sections(target_exam_id uuid, ordered_section_ids uuid[])
...
begin
  ...
  update public.exam_sections set position = position + 100000 where exam_id = target_exam_id and deleted_at is null;
...
create or replace function public.reorder_section_questions(target_section_id uuid, ordered_question_ids uuid[])
returns table (success boolean, code text)
language plpgsql
security definer
set search_path = public
...
begin
  ...
  update public.questions set position = position + 100000 where section_id = target_section_id and deleted_at is null;
...
create or replace function public.reorder_question_options(target_question_id uuid, ordered_option_ids uuid[])
returns table (success boolean, code text)
language plpgsql
security definer
set search_path = public
...
begin
  ...
  update public.question_options set position = position + 100000 where question_id = target_question_id and deleted_at is null;
