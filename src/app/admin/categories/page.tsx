...
export default async function Page({
  ...
  const { items, count, params } = await listCategories(resolved);
  const totalPages = Math.max(1, Math.ceil(count / params.pageSize));

  return (
    <div className="space-y-8 pb-12">
      <PageHeader
        title="Quản lý danh mục kỳ thi"
        description="Phân loại các đề thi theo kỳ thi cụ thể như HSA, ĐGNL, Tốt nghiệp THPT, ..."
        breadcrumbs={[
      ...
      <Card className="border-[var(--border)] bg-[var(--card)] rounded-2xl shadow-xl">
        <CardContent className="p-5 sm:p-6">
          <form className="flex flex-wrap items-end gap-3" action="/admin/categories">
      ...
      <Card className="border-[var(--border)] bg-[var(--card)] rounded-2xl shadow-xl overflow-hidden">
        <CardContent className="p-0 overflow-x-auto">
          <Table>
            <thead>
              <tr>
                <Th className="whitespace-nowrap min-w-[200px]">Tên danh mục</Th>
              ...
              {items.map((item) => (
                <tr key={item.id} className="hover:bg-[var(--surface-hover)]">
                  ...
                  <Td className="whitespace-nowrap text-right">
                    <TaxonomyActions item={item} type="category" />
                  </Td>
                </tr>
              ))}
              {items.length === 0 && (
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-[var(--muted-foreground)]">
        <span>
          Trang {params.page} / {totalPages} — Tổng cộng {count} danh mục.
        </span>
        {totalPages > 1 && (
