import Link from "next/link";
import { Plus, Search, Filter } from "lucide-react";
import { PageHeader } from "@/components/shared/page-header";
import { StatusBadge } from "@/components/shared/status-badge";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import { Input } from "@/components/ui/input";
import { Table, Td, Th } from "@/components/ui/table";
import { ExamRowActions } from "@/components/admin/exam-row-actions";
import type { SearchParamsRecord } from "@/lib/admin/types";
import { listActiveCategories } from "@/lib/categories/queries";
import { accessTypeLabels, examStatusLabels, listExams } from "@/lib/exams/queries";
import { listActiveSubjects } from "@/lib/subjects/queries";

export default async function Page({
  searchParams,
}: {
  searchParams: Promise<SearchParamsRecord>;
}) {
  const resolved = await searchParams;
  const [{ items, count, params }, subjects, categories] = await Promise.all([
    listExams(resolved),
    listActiveSubjects(),
    listActiveCategories(),
  ]);
  const totalPages = Math.max(1, Math.ceil(count / params.pageSize));

  return (
    <div className="space-y-8 pb-12">
      <PageHeader
        title="Quản lý đề thi"
        description="Tạo mới, cấu hình thời gian, điểm số và quản lý trạng thái xuất bản các đề thi."
        breadcrumbs={[
          { label: "Quản trị", href: "/admin" },
          { label: "Đề thi" },
        ]}
        actions={
          <Button asChild className="shrink-0 bg-[var(--primary)] hover:bg-[var(--primary-hover)] text-white rounded-xl shadow-md shadow-blue-600/20">
            <Link href="/admin/exams/new" className="flex items-center gap-1.5">
              <Plus className="h-4 w-4" />
              <span>Tạo đề mới</span>
            </Link>
          </Button>
        }
      />

      {/* Filter and Search Bar */}
      <Card className="border-[var(--border)] bg-[var(--card)] rounded-2xl shadow-xl">
        <CardContent className="p-5 sm:p-6">
          <form className="flex flex-wrap items-end gap-3" action="/admin/exams">
            <label className="flex flex-col gap-1.5 text-xs font-semibold text-[var(--foreground)] min-w-[220px] flex-1 sm:flex-initial">
              <span>Từ khóa tìm kiếm</span>
              <div className="relative">
                <Search className="pointer-events-none absolute left-3 top-3 h-4 w-4 text-[var(--muted-foreground)]" />
                <Input
                  name="q"
                  defaultValue={params.q}
                  placeholder="Tìm theo tiêu đề hoặc slug..."
                  className="pl-9 text-sm bg-[var(--input-bg)] border-[var(--input-border)] text-[var(--foreground)]"
                />
              </div>
            </label>

            <label className="flex flex-col gap-1.5 text-xs font-semibold text-[var(--foreground)] min-w-[140px]">
              <span>Trạng thái</span>
              <select
                name="status"
                defaultValue={params.status}
                className="h-10 rounded-xl border border-[var(--input-border)] bg-[var(--input-bg)] px-3 text-sm text-[var(--foreground)] focus:border-[var(--primary)] focus:outline-none"
              >
                <option value="all">Tất cả trạng thái</option>
                {Object.entries(examStatusLabels).map(([value, label]) => (
                  <option key={value} value={value}>
                    {label}
                  </option>
                ))}
              </select>
            </label>

            <label className="flex flex-col gap-1.5 text-xs font-semibold text-[var(--foreground)] min-w-[150px] max-w-[200px]">
              <span>Môn học</span>
              <select
                name="subjectId"
                defaultValue={params.subjectId}
                className="h-10 w-full truncate rounded-xl border border-[var(--input-border)] bg-[var(--input-bg)] px-3 text-sm text-[var(--foreground)] focus:border-[var(--primary)] focus:outline-none"
              >
                <option value="all">Tất cả môn học</option>
                {subjects.map((item) => (
                  <option key={item.id} value={item.id}>
                    {item.name}
                  </option>
                ))}
              </select>
            </label>

            <label className="flex flex-col gap-1.5 text-xs font-semibold text-[var(--foreground)] min-w-[150px] max-w-[200px]">
              <span>Danh mục</span>
              <select
                name="categoryId"
                defaultValue={params.categoryId}
                className="h-10 w-full truncate rounded-xl border border-[var(--input-border)] bg-[var(--input-bg)] px-3 text-sm text-[var(--foreground)] focus:border-[var(--primary)] focus:outline-none"
              >
                <option value="all">Tất cả danh mục</option>
                {categories.map((item) => (
                  <option key={item.id} value={item.id}>
                    {item.name}
                  </option>
                ))}
              </select>
            </label>

            <Button type="submit" variant="outline" className="h-10 px-4 shrink-0 border-[var(--border)] bg-[var(--surface)] text-[var(--foreground)] hover:bg-[var(--surface-hover)] rounded-xl">
              <Filter className="mr-1.5 h-3.5 w-3.5" /> Lọc
            </Button>
          </form>
        </CardContent>
      </Card>

      {/* Main Table */}
      <Card className="border-[var(--border)] bg-[var(--card)] rounded-2xl shadow-xl overflow-hidden">
        <CardContent className="p-0 overflow-x-auto">
          <Table>
            <thead>
              <tr>
                <Th className="whitespace-nowrap min-w-[220px]">Tiêu đề</Th>
                <Th className="whitespace-nowrap min-w-[120px]">Môn học</Th>
                <Th className="whitespace-nowrap min-w-[120px]">Danh mục</Th>
                <Th className="whitespace-nowrap min-w-[120px]">Trạng thái</Th>
                <Th className="whitespace-nowrap min-w-[140px]">Quyền truy cập</Th>
                <Th className="whitespace-nowrap min-w-[100px]">Thời gian</Th>
                <Th className="whitespace-nowrap min-w-[90px]">Tổng điểm</Th>
                <Th className="whitespace-nowrap min-w-[110px]">Cập nhật</Th>
                <Th className="whitespace-nowrap text-right min-w-[90px]">Hành động</Th>
              </tr>
            </thead>
            <tbody>
              {items.map((exam) => (
                <tr key={exam.id} className="hover:bg-[var(--surface-hover)]">
                  <Td className="font-semibold text-[var(--foreground)] min-w-[220px]">
                    {exam.title}
                  </Td>
                  <Td className="whitespace-nowrap text-[var(--muted-foreground)]">
                    {exam.subjects?.name ?? "Chưa có"}
                  </Td>
                  <Td className="whitespace-nowrap text-[var(--muted-foreground)]">
                    {exam.exam_categories?.name ?? "Không chọn"}
                  </Td>
                  <Td className="whitespace-nowrap">
                    <StatusBadge status={exam.status} />
                  </Td>
                  <Td className="whitespace-nowrap text-xs text-[var(--muted-foreground)]">
                    {accessTypeLabels[exam.access_type]}
                  </Td>
                  <Td className="whitespace-nowrap text-xs text-[var(--muted-foreground)] font-medium">
                    {exam.duration_minutes} phút
                  </Td>
                  <Td className="whitespace-nowrap font-bold text-[var(--foreground)]">
                    {exam.total_score}
                  </Td>
                  <Td className="whitespace-nowrap text-xs text-[var(--muted-foreground)]">
                    {new Date(exam.updated_at).toLocaleDateString("vi-VN")}
                  </Td>
                  <Td className="whitespace-nowrap text-right">
                    <ExamRowActions exam={exam} />
                  </Td>
                </tr>
              ))}
              {items.length === 0 && (
                <tr>
                  <Td colSpan={9} className="text-center py-12 text-[var(--muted-foreground)]">
                    Chưa có đề thi nào phù hợp với điều kiện tìm kiếm.
                  </Td>
                </tr>
              )}
            </tbody>
          </Table>
        </CardContent>
      </Card>

      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-[var(--muted-foreground)]">
        <span>
          Trang {params.page} / {totalPages} — Tổng cộng {count} đề thi.
        </span>
        {totalPages > 1 && (
          <div className="flex items-center gap-2">
            {params.page > 1 && (
              <Button asChild variant="outline" size="sm" className="h-8 text-xs border-[var(--border)] text-[var(--foreground)] hover:bg-[var(--surface-hover)] rounded-xl">
                <Link
                  href={{
                    pathname: "/admin/exams",
                    query: { ...resolved, page: params.page - 1 },
                  }}
                >
                  Trang trước
                </Link>
              </Button>
            )}
            {params.page < totalPages && (
              <Button asChild variant="outline" size="sm" className="h-8 text-xs border-[var(--border)] text-[var(--foreground)] hover:bg-[var(--surface-hover)] rounded-xl">
                <Link
                  href={{
                    pathname: "/admin/exams",
                    query: { ...resolved, page: params.page + 1 },
                  }}
                >
                  Trang sau
                </Link>
              </Button>
            )}
          </div>
        )}
      </div>
    </div>
  );
}
