...
export default async function Page({ searchParams }: { searchParams: Promise<SearchParamsRecord> }) {
  const { items, count, params } = await listCategories(await searchParams);
  const totalPages = Math.max(1, Math.ceil(count / params.pageSize));
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <form className="flex flex-col gap-2 md:flex-row md:items-end" action="/admin/categories">
        ...
        <ModalShell title="Tạo danh mục kỳ thi" trigger={<Button>Tạo danh mục</Button>}><TaxonomyForm type="category" /></ModalShell>
      </div>
      <Card>
        <CardHeader><CardTitle>Danh mục kỳ thi</CardTitle></CardHeader>
        <CardContent className="overflow-x-auto">
          <Table>
            <thead><tr><Th>Tên danh mục</Th><Th>Slug</Th><Th>Trạng thái</Th><Th>Ngày cập nhật</Th><Th>Hành động</Th></tr></thead>
            <tbody>
              {items.map((item) => (
                <tr key={item.id}>
                  ...
                  <Td><TaxonomyActions item={item} type="category" /></Td>
                </tr>
              ))}
              {items.length === 0 && <tr><Td colSpan={5}>Chưa có dữ liệu.</Td></tr>}
            </tbody>
          </Table>
          <p className="mt-4 text-sm text-[var(--muted-foreground)]">Trang {params.page} / {totalPages} - Tổng cộng {count} danh mục.</p>
        </CardContent>
      </Card>
    </div>
  );
